...
- 일정: 포커스된 달의 일정과 특일을 4개 탭(나의 일정 / 공휴일 / 기념일 / 전체)으로 분류하여 표시하는 월간 목록 팝업; 좌우 스와이프로 탭 전환 및 이전·다음 달 이동 지원; 사잇날 자동 감지 후 공휴일·전체 탭에 초록색으로 표시
...
- 매일 / 매주 / 매월 / 매년 반복 지원
- **매주 반복**: 등록 날짜의 요일 기준, 해당 월의 동일 요일에 모두 표시
...
- 반복 알림: 매일(`time`), 매주(`dayOfWeekAndTime`), 매월(`dayOfMonthAndTime`) — 매년은 단발 알림
